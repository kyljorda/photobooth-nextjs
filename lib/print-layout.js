...
export function computeStripLayout(opts = {}) {
  ...
  return {
    ...
    safeInset: safe,
    caption: {
      // Centre the date within the band left below the frames.
      centerX: Math.round(W / 2),
      baselineY: Math.round(captionTop + (H - captionTop) / 2),
      maxWidth: W - side * 2,
      // Scale type with DPI so it is legible at any resolution.
      fontPx: Math.max(10, Math.round(0.055 * dpi)),
    },
...
export function assertLayoutFits(layout) {
  ...
  if (last && last.y + last.h > layout.caption.baselineY) {
